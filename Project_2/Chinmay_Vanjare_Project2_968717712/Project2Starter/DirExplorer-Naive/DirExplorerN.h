...
namespace FileSystem
{
  ...
  public:
	...
	std::vector<std::string> filepathReturn();
  ...
  */
  ...
  inline void DirExplorerN::doFile(const std::string& filename)
  {
    ++fileCount_;
    std::cout << "\n  --   " << filename;
	address.push_back(Directory::getCurrentDirectory()+"\\"+filename);
  }
  ...
  //----< return the vector storing names of all files >-------------

  inline std::vector<std::string> DirExplorerN::filepathReturn()
  {
	  return address;
  }
}