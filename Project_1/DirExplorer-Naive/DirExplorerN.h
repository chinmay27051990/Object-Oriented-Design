...
namespace FileSystem
{
  ...
  public:
	...
	using RegexFiles = std::vector<std::string>;
	...
	void RegexMatch(std::regex reg, std::vector<std::string> file);
	std::vector<std::string> StorageFiles();
	std::vector<std::string> StorageFilesFiltered();
  ...
  private:
    ...
    size_t fileCount_ = 0;
    bool recurse_ = false;
	std::vector<std::string> regexfiles_;
  ...
  */
  inline void DirExplorerN::find(const std::string& path)
  {
    ...
    for (auto patt : patterns_)
    {
      ...
      for (auto f : files)
      {
		...
		searchedfiles.push_back(fpath + "\\" + f);
      }
    }
  ...
  //--find files which matched regexes that defined in process command line regexes
  inline void DirExplorerN::RegexMatch(std::regex reg, std::vector<std::string> file)
  {
	 /* auto filesBeforeregex = file;
	  regexfiles_.clear();
	  for (auto files : filesBeforeregex) {
		  std::string getfilename = Path::getName(files);
			  if (std::regex_match(getfilename,reg)) {
				  regexfiles_.push_back(files);
				  
		  }

	  }*/
  }
  //--store files in directory  that match in process command line path obtion pattern and regexes.
  inline std::vector<std::string> DirExplorerN::StorageFiles()
  {
	  return searchedfiles;
  }

  inline std::vector<std::string> DirExplorerN::StorageFilesFiltered()
  {
	  return regexfiles_;
  }
